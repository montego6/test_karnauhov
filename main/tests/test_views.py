...
@pytest.mark.parametrize('from_p,to_p,amount_p', [
    ('usd', 'rub', 1),
    ('rub', 'usd', 1000),
    ('usd', 'eur', 1.5)
    ])
def test_convert_api_success_view(from_p, to_p, amount_p):
    response = client.get(reverse('api-convert'), {
        consts.FROM: from_p,
        consts.TO: to_p,
        consts.AMOUNT: amount_p
    })
...
@pytest.mark.parametrize('query_params,code,detail', [
    ({consts.FROM: 'usd', consts.AMOUNT: 1}, 400, consts.QUERY_ERROR_DETAIL % consts.TO.upper()),
    ({consts.TO: 'usd', consts.AMOUNT: 1}, 400, consts.QUERY_ERROR_DETAIL % consts.FROM.upper()),
    ({consts.AMOUNT: 1}, 400, consts.QUERY_ERROR_DETAIL % consts.FROM.upper()),
    ({consts.FROM: 'usd', consts.TO: 'eur'}, 400, consts.QUERY_ERROR_DETAIL % consts.AMOUNT.upper()),
    ({}, 400, consts.QUERY_ERROR_DETAIL % consts.FROM.upper()),
    ({consts.FROM: 'abreg', consts.TO: 'usd',consts.AMOUNT: 1}, 400, consts.CURRENCY_ERROR_DETAIL),
    ({consts.FROM: 'eur', consts.TO: 'zx1',consts.AMOUNT: 1}, 400, consts.CURRENCY_ERROR_DETAIL),
])
